...
async function main() {
  ...
  if (utils.isAddress(paymasterAddress)) {
    ...
    await byzanlinkAASdk.addUserOpsToBatch({ to: tokenAddress, data: encodedData });
    const approveOp = await byzanlinkAASdk.estimate();
    console.log(`UserOpHash: ${await printOp(approveOp)}`);
    const uoHash1 = await byzanlinkAASdk.send(approveOp);
    ...
    while ((userOpsReceipt1 == null) && (Date.now() < timeout1)) {
      await sleep(2);
      userOpsReceipt1 = await byzanlinkAASdk.getUserOpReceipt(uoHash1);
    }
